...
def allocate_funds(issues, budget, funding_limit):


    proposals = [(issue['project_name'], issue['amount_requested'], issue['funded_amount']) for issue in issues]
    funded = select_proposals_to_fund(budget, funding_limit, proposals)
...
def main():
    ...
    allocate_funds(sdg_issues_round, arguments.budget, arguments.funding_limit)
